...
<!--      </view>-->
      ...
      <!-- 分享更多 -->
      <view v-if="false" class="share-more">
    ...
    <view class="banner-title">
      <!-- banner -->
      <view class="banner">
        <u-swiper :list="bannerList" height="750"></u-swiper>
<!--        <swiper class="screen-swiper round-dot" indicator-dots="true" circular="true" autoplay="true" interval="5000"-->
<!--                duration="500">-->
<!--          <swiper-item v-for="(item, index) in swiperList" :key="index">-->
<!--            <image :src="item.url" mode="aspectFill"></image>-->
<!--            &lt;!&ndash; <video src="{{item.url}}" autoplay loop muted show-play-btn="{{false}}" controls="{{false-->
<!--            }}" objectFit="cover" wx:if="{{item.type == 'video'}}"></video> &ndash;&gt;-->
<!--          </swiper-item>-->
<!--        </swiper>-->
      </view>
      <!-- 价格 -->
      <view class="price-info" v-if="goodsInfo" v-show="type==0">
        <view class="price">
          <text class="min">￥</text>
          <text class="max">{{goodsInfo.zk_final_price}}</text>
        ...
        <view class="info" v-if="false">
          ...
          <view class="list" @click="onAttention">
			...
			<!-- 限时抢购 -->
			<view v-if="false" class="flash-price" v-show="type==1">
      ...
      <!-- 标题 -->
      <view v-if="goodsInfo" class="goods-title">
        <text>{{goodsInfo.title}}</text>
      </view>
      <!-- 开通会员 -->
      <view v-if="false" class="dredge-vip" >
        <view class="title">
    ...
    <!-- 优惠积分 -->
    <view v-if="false" class="goods-discounts">
      <view class="list">
    ...
    <!-- 评价 -->
    <view v-if="false" class="evaluate-data" ref="evaluate">
    ...
    <!-- 排行榜 -->
    <view v-if="false" class="ranking-list">
    ...
    <!-- 商品介绍 -->
    <view v-if="false" class="products-introduction" ref="products">
      <view class="title">
        <text>商品介绍</text>
      </view>
    ...
    <!-- 底部 -->
    <view v-if="false" class="page-footer">
...
export default {
  ...
  data() {
	...
	onLoad(params) {
        log.debug(decodeURIComponent(params.goodsInfo))



        this.couponShareUrl = decodeURIComponent(params.couponShareUrl);
        this.couponStartFee = decodeURIComponent(params.couponStartFee);
        this.couponTotalCount = decodeURIComponent(params.couponTotalCount);
        this.couponRemainTime = decodeURIComponent(params.couponRemainTime);
        this.couponStartTime = decodeURIComponent(params.couponStartTime);
        this.couponEndTime = decodeURIComponent(params.couponEndTime);
        this.couponAmount = decodeURIComponent(params.couponAmount);
        this.itemId = decodeURIComponent(params.itemId);
		this.type = params.type||0;
		api.tbk.getItemInfo(params.itemId).then(res => {
        if (res.code == api.SUCCESS) {
          this.goodsInfo = res.data[0];
          this.formatGoodsInfo();
        }
      })
      // api.tbk.getItemCoupon(params.itemId).then(res => {
      //   if (res.code == api.SUCCESS) {
  ...
  methods: {
    ...
    formatGoodsInfo(){
        log.debug(this.goodsInfo);
        this.swiperList = [];
        this.bannerList = this.goodsInfo.small_images.string;
        for(var i=0; i<this.goodsInfo.small_images.string.length; i++) {
          this.swiperList.push({
            url: this.goodsInfo.small_images.string[i],
          })
        }
    },
    /**
