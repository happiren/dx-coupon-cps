	...
	methods: {
		...
		save(e) {
			...
			uni.authorize({
				...
				success() {
					console.log('dx-previewImage:允许储存');
		...
		downloadImg(src) {
			...
			uni.downloadFile({
				...
				success: function(res) {
					console.log('dx-previewImage:下载成功');
					uni.hideLoading();
		...
		open(e) {
			if (e === null || e === '') {
				console.log('dx-previewImage:打开参数无效');
				return;
			}
			...
			if (!isNaN(e)) {
				...
				if(index===-1){
					this.imgs = [e];
					this.index = 0;
					console.log('dx-previewImage:未在图片地址数组中找到传入的图片，已为你自动打开单张预览模式')
				}else{
					this.index = this.imgs.indexOf(e);
				}
			}
			console.log('dx-previewImage:当前预览图片序号'+this.index);
			this.show = true;
		},
		//关闭
		close(e) {
