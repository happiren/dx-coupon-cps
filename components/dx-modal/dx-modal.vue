	...
	<view>
		<u-popup :zoom="zoom" mode="center" :popup="false" :z-index="uZIndex" v-model="value" :length="width"
		 :mask-close-able="maskCloseAble" :border-radius="borderRadius" @close="popupClose" :negative-top="negativeTop">
			<view class="dx-model">
				<view v-if="showTitle" class="dx-model__title u-line-1" :style="[titleStyle]">{{ title }}</view>
				<view class="dx-model__content">
					<view :style="[contentStyle]" v-if="$slots.default  || $slots.$default">
						<slot />
					</view>
					<view v-else class="dx-model__content__message" :style="[contentStyle]">{{ content }}</view>
				</view>
				<view class="dx-model__footer" v-if="showCancelButton || showConfirmButton">
					<view v-if="showCancelButton" class="cancel-btn" :hover-stay-time="100" hover-class="dx-model__btn--hover"
					  @tap="cancel">
						<view class="" :style="[cancelBtnStyle]">
							{{cancelText}}
						</view>
					</view>
					<view v-if="showConfirmButton || $slots['confirm-button']" :hover-stay-time="100" :hover-class="asyncClose ? 'none' : 'dx-model__btn--hover'"
					 class="confirm-btn"  @tap="confirm">
						<slot v-if="$slots['confirm-button']" name="confirm-button"></slot>
						<block v-else>
							<u-loading mode="circle" :color="confirmColor" v-if="loading"></u-loading>
							<block v-else>
								<view class="" :style="[confirmBtnStyle]">
									{{confirmText}}
								</view>
							</block>
						</block>
...
<script>
	...
	export default {
		...
		computed: {
			cancelBtnStyle() {
				return Object.assign({
					...
					background: this.cancelBackgroundColor,
					borderRadius: "8rpx",
				}, this.cancelStyle);
			},
			confirmBtnStyle() {
				return Object.assign({
					...
					backgroundColor: this.confirmBackgroundColor,
					borderRadius: "8rpx",
				}, this.confirmStyle);
			},
			uZIndex() {
		...
		methods: {
			init(config) {
				console.log("dx modal init", config)
